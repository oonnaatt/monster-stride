...
function getTypeScores(activities: Activity[]): Map<RemnonType, number> {
  ...
  const addScore = (type: RemnonType, amount: number) => {
  ...
  for (const act of activities) {
    ...
    // Biome
    const biomeMap: Record<string, RemnonType> = {
...
function calculateStats(types: RemnonType[]): { attack: number; defense: number; speed: number } {
  if (types.length === 0) {
...
export async function hatchRemnon(userId: string, supabase: SupabaseClient): Promise<Remnon> {
